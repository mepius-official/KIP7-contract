...
contract KIP7 is KIP13, IKIP7 {
    ...
    // lock time for each token
    mapping (address => uint256) private _locktime;
    // lock amount for each token
    ...
    function balanceOf(address account) public view returns (uint256) {
        return _balances[account] + _locks[account];
    }
    ...
    function lockTimeOf(address account) public view returns (uint256) {
        return _locktime[account];
    }

    function lockToTime(address account, uint256 amount, uint256 lockTime) public returns (bool) {
        require(amount > 0, "amount must be greater than 0");
        require(unlockBalanceOf(account) >= amount, "Not enough balance");
        require(lockTime > block.timestamp, "Lock time should be greater than current time");

        _balances[account] -= amount;
        _locks[account] += amount;
        _locktime[account] = lockTime;

        return true;
    }

    function autoUnlock(address _holder) public returns (bool) {
        if (_locktime[_holder] <= block.timestamp) {
    ...
    /**
     * Unlock function
     * _locktime[account] is the time when the lock is released
     * check _lock[account] is not 0
     */
    function releaseTimeLock(address _holder) public returns (bool) {
        _balances[_holder] += _locks[_holder];
        _locks[_holder] = 0;
        _locktime[_holder] = 0;
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        if (_locks[msg.sender] > 0) {
            autoUnlock(msg.sender);
        }

        require(amount <= unlockBalanceOf(msg.sender), "transfer amount exceeds balance");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
        ...
        require(amount <= unlockBalanceOf(sender), "transferFrom amount exceeds balance");
        require(amount <= allowance(sender, msg.sender), "transferFrom amount exceeds allowance");
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(amount >= 0, "transferFrom amount must be greater than 0");
...
contract KIP7Pausable is KIP13, KIP7, Pausable {
    ...
    function lockToTime(address account, uint256 amount, uint256 lockTime) public whenNotPaused returns (bool) {
        return super.lockToTime(account, amount, lockTime);
    }

    function autoUnlock(address account) public whenNotPaused returns (bool) {
        return super.autoUnlock(account);
    }
    
    function releaseTimeLock(address account) public whenNotPaused returns (bool) {
        return super.releaseTimeLock(account);
    }
...
contract KIP7Burnable is KIP13, KIP7 {
    ...
    function burnFrom(address account, uint256 amount) public {
        require(account != address(0), "KIP7: burn from the zero address");
        require(amount <= allowance(account, msg.sender), "KIP7: burn amount exceeds allowance");
        require(amount > 0, "KIP7: burn amount must be greater than 0");
 ...
 * See http://kips.klaytn.com/KIPs/kip-7-fungible_token
 */
contract KIP7Mintable is KIP13, KIP7, MinterRole, Pausable {
    /*
    ...
    function lockToTime(address account, uint256 amount, uint256 lockTime) public onlyMinter returns (bool) {
        require(paused() == false, "KIP7: contract is paused");
        return super.lockToTime(account, amount, lockTime);
    }
...
/* MEPS Token Impl   */
/*********************/
contract MEPS is KIP7Token {
    string name_ = "Mepius";
    string symbol_ = "MEPS";
    uint8 decimals_int = 10;
    uint256 _amount = 5827500000;
    uint256 public init_supply = _amount * (10 ** uint256(decimals_int));
    
    constructor() KIP7Token(name_, symbol_, decimals_int, init_supply) public {
    }
}
